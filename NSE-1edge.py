...
###################################################################
PATH = 'DataBase/OGB_MAG2500/FP.txt'
type2idx = {'F': 0, 'P': 1}
# Struct Encoder#######################################################################
type_list = ['F', 'P']
# Feature Injection#######################################################################
...
def dist_encoder(src, dest, G, K_HOP):
    # 这个是起点到终点的情况编码，若src=dest，则[0.,0.]；若src到得了dest，则[0.,1.]；若src到得了dest，
...
def AttInGCN(fea, model='meirec'):
    """
    :param fea: fea_batch, [[0, 0, 4], [[0, 1, 1], [0, 1, 4], [0, 1, 4], [0, 1, 1], [1, 0, 1]]]
    :return:
    """
    with tf.variable_scope(name_or_scope='AttInGCN', reuse=tf.AUTO_REUSE):
        # 提取node、1阶邻居特征、2阶邻居特征
        node = fea[:, :NUM_FEA]
        neigh1 = fea[:, NUM_FEA:NUM_FEA * (NUM_NEIGHBOR + 1)]
        neigh1 = tf.reshape(neigh1, [-1, NUM_NEIGHBOR, NUM_FEA])
        if model == 'meirec':
            order1_nodemean = tf.reduce_mean(neigh1, axis=1)
            tmp = tf.concat([node, order1_nodemean], axis=1)
...
def LinkPrediction(n1, n2, label):
    # 通过调用函数AttInGCN(n1)和AttInGCN(n2)，分别对节点n1和n2进行图神经网络（AttInGCN）的嵌入操作
    n1_emb = AttInGCN(n1)
    n2_emb = AttInGCN(n2)
...
A_holder = tf.placeholder(tf.float32, shape=(None, NUM_FEA + NUM_FEA * NUM_NEIGHBOR),
                          name='a')
B_holder = tf.placeholder(tf.float32, shape=(None, NUM_FEA + NUM_FEA * NUM_NEIGHBOR),
                          name='b')
