...
def gen_fea_batch1(G, root, des, fea_dict, K_HOP):
    ...
    if USE_DE:
        ...
        a = [0.0] * (8 + 2) * K_HOP + type_encoder(root)
    else:
        a = type_encoder(root)
...
def subgraph_sampling_with_DE_node_pair(G, node_pair, K_HOP=2):
    ...
    B_fea_batch = gen_fea_batch1(sub_G_for_AB, A, B,
                                 SPD_based_on_node_pair, K_HOP)
...
def AttInGCN(fea, model='meirec'):
    ...
    with tf.variable_scope(name_or_scope='AttInGCN', reuse=tf.AUTO_REUSE):
        ...
        if model == 'meirec':
            ...
            order1_nodemean = tf.reduce_mean(tmp, axis=1)
            emb = tf.concat([node, order1_nodemean], axis=1)
